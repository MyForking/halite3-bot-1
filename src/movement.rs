...
pub fn greedy(state: &mut GameState, ship_id: ShipId) -> Direction {
    ...
    if mov.is_none() && current_halite < SEEK_LIMIT {
        mov = state
            .get_nearest_halite_move(pos, SEEK_LIMIT)
            .map(|d| (d, pos.directional_offset(d)));
        if let Some((_, p)) = mov {
            Log::log(&format!(
                "greedy ship {:?} found new target: {:?}.",
                ship_id, p
            ));
        } else {
            Log::log(&format!(
                "greedy ship {:?} does not know where to go.",
                ship_id
            ));
        }
    }
...
pub fn thorough(state: &mut GameState, ship_id: ShipId) -> Direction {
    ...
    let mov = state
        .get_nearest_halite_move(pos, HARVEST_LIMIT)
        .map(|d| (d, pos.directional_offset(d)));
    if let Some((_, p)) = mov {
        Log::log(&format!(
            "thorough ship {:?} found new target: {:?}.",
            ship_id, p
        ));
    } else {
        Log::log(&format!(
            "thorough ship {:?} does not know where to go.",
            ship_id
        ));
    }
    ...
    pub fn new() -> Self {
        CollectorNeuralNet {
            ...
            layer1_weights: vec![
                ...
                0.49475350019076497,
                -0.19978128790616811,
                0.018098899484667925,
                -0.18056271245211516,
                0.04917608608793759,
                0.03745633448405858,
                0.12247604713830554,
                -0.08804248036205618,
                -0.048857370620712715,
                -0.17239977110345875,
                -0.5193864382068352,
                0.136208019047245,
                -0.307380181072233,
                ...
                -0.07589348760749239,
                -2.036463123719775,
                0.021736913880338233,
                1.4017034961879342,
                0.7203467649161311,
                -0.08021554713242149,
                0.5053933688993788,
                0.29639800124440435,
                0.050676549376177445,
                -0.2893032301256404,
                -1.556673786607969,
                1.3316669915814008,
                2.551832692976115,
